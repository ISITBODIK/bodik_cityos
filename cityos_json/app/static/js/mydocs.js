let dataset_list = null;
let myconfig_list = {};
let selected_myconfig = null;

function show_page() {
    let result = false;
    try {
        let check_keyvalues = document.getElementById('keyvalues');
        check_keyvalues.setAttribute('title', 'keyValues形式で返す');
        let check_method = document.getElementById('method');
        check_method.setAttribute('title', 'POSTメソッドで検索する');

        let search_button = document.getElementById('search');
        search_button.addEventListener('click', () => {
            execute_search();
        });

        get_dataset_list();

        show_data([]);

    } catch(error) {
        alert('show_page:' + error);
    }
}

function get_dataset_list() {
    try {
        let headers = {
            'User-Agent': 'BODIK/2.0',
            'accept': 'application/json'
        }
        let options = {
            headers: headers
        }
        let url = `${fiware_server}/myapi/public`;
        fetch(url, options)
        .then(response => response.json())
        .then(data => {
            set_dataset_select(data);
        });

    } catch(error) {
        alert('get_dataset_list:' + error);
    }
}

function set_dataset_select(apilist) {
    try {
        let combo = clear_child('select_dataset');
        if (combo) {
            dataset_list = {};            
            for (let info of apilist) {
                let apiname = info.entity_type;
                dataset_list[apiname] = info;
                
                let option = document.createElement('option');
                option.value = apiname;
                option.text = info.display_name;
                combo.appendChild(option);
            }
            combo.selectedIndex = 0;

            combo.addEventListener('change', (evt) => {
                select_dataset();
            });

            select_dataset();
        }
    } catch(error) {
        alert('set_dataset_select:' + error)
    }
}

function select_dataset() {
    try {
        let combo = document.getElementById('select_dataset');
        if (combo) {
            selected_apiname = combo.value;
            if (selected_apiname in myconfig_list) {
                selected_myconfig = myconfig_list[selected_apiname];
                show_params();
            } else {
                get_myconfig();
            }
        }
    } catch(error) {
        alert('select_dataset:' + error)
    }
}

function get_myconfig() {
    try {
        let info = dataset_list[selected_apiname];
        let apiname = info['apiname'];
        let headers = {
            'User-Agent': 'BODIK/2.0',
            'accept': 'application/json'
        }
        let options = {
            headers: headers
        }
        let url = `${fiware_server}/myapi/config?apiname=${apiname}`;
        fetch(url, options)
        .then(response => response.json())
        .then(data => {
            myconfig_list[selected_apiname] = data;
            selected_myconfig = data;
            show_params();
        });
    } catch(error) {
        alert('select_dataset:' + error)
    }
}

function show_params() {
    try {
        let area = clear_child('param_area');
        if (area) {
            if (selected_myconfig) {
                let datamodel = selected_myconfig['dataModel'];
                for (let field in datamodel) {
                    let info = datamodel[field];
                    let row = makebox(area, 'row', '', '');
                    makebox(row, 'caption', '', field);
                    makeEdittext(row, 'text_field', field);
                }
                //makebox(area, 'row_last', 'dummy', '');
            }
        }
    } catch(error) {
        alert('show_params:' + error);
    }
}

function execute_search() {
    try {
        call_ngsi({});
    } catch(error) {
        alert('execute_search:' + error);
    }
}

async function call_ngsi() {
    try {
        show_data(null);
        let entity_type = selected_myconfig.entity_type;
        let fiware_service = selected_myconfig['Fiware-Service'];
        let fiware_servicepath = selected_myconfig['Fiware-ServicePath'];

        let request_body = clear_child('request_body');
        let method_post = document.getElementById('method').checked;

        let q = {
            //'type': entity_type,
            'offset': 0,
            'limit': 100
        }

        let keyvalue = document.getElementById('keyvalues').checked;
        if (keyvalue) {
            q['options'] = 'keyValues';
        }

        let headers = {
            'User-Agent': 'cityos/2.0',
            'Accept': 'application/json',
            'Fiware-Service': fiware_service,
            'Fiware-ServicePath': fiware_servicepath
        }

        let options = {};
        let url = '';
        let params = {};
        if (method_post) {
            const qp = new URLSearchParams(q);
            url = fiware_server + '/v2/op/query' + '?' + qp.toString();

            headers['Content-Type'] = 'application/json; charset=utf-8';

            params = {
                'entities': [
                    {
                        "idPattern": ".*",
                        "type": entity_type
                    }
                ],
                //"expression": expression
            }
            let expression = {};

            options['method'] = 'POST'
            options['headers'] = headers;
            options['body'] = JSON.stringify(params)
        } else {
            q['type'] = entity_type;
            const qp = new URLSearchParams(q);
            url = fiware_server + '/v2/entities' + '?' + qp.toString();
            options['headers'] = headers;
        }
        console.log(JSON.stringify(params, 0, 2));
        let request_text = '<B>Request URL</B>\n' + url + '\n\n';
        request_text += '<B>Request Headers</B>\n' + JSON.stringify(headers, 0, 2) + '\n\n';
        if (method_post) {
            request_text += '<B>Request Body</B>\n' + JSON.stringify(params, 0, 2);
        }
        request_body.innerHTML = request_text;

        let resp = await fetch(url, options);
        let status_code = resp.status;
        let expired = false;
        if (status_code == 200) {
            show_data(await resp.json());
        } else {
            alert('エラーです:' + status_code);
        }
    } catch(error) {
        alert('call_ngsi:' + error);
    }
}

function show_data(data) {
    try {
        let body = clear_child('response_body');
        body.innerText = JSON.stringify(data, null, 2);
    } catch(error) {
        alert('show_data:' + error);
    }
}
