...
class MyConfig:
    ...
    @classmethod
    def delete_config(cls, apiname):
        result = False
        try:
            doc_id = apiname
            if myes.get_source(cls.config, doc_id):
                myes.delete_document_id(cls.config, doc_id)

            result = True

        except Exception as e:
            print('ERROR delete_config', e)
        return result

    @classmethod
    def search_public_api(cls, isLocation):
        result = False
        try:
            condition = [
                { "wildcard": { "Fiware-ServicePath": "/public*" }}
            ]
            if isLocation:
                condition.append(
                    { "term": { "geometry": "Point" }}
            ...
            qs = {
                ...
                "_source": [ "apiname", "display_name", "geometry", "entity_type", "Fiware-Service", "Fiware-ServicePath"]
            }
